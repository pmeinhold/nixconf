{ ... }:
  # Check: https://github.com/kmonad/kmonad/blob/master/doc/faq.md#q-how-do-i-get-uinput-permissions
{
  flake.modules.nixos.feature-kmonad = { pkgs, ... }: {
  ...
  flake.modules.homeManager.feature-kmonad-miryoku = { lib, config, pkgs, ... }:
  let
    filepath = ".config/kmonad/miryoku_lite.kbd";
  in
  {
    options.feature.kmonad.miryoku = {
      thumbRow = lib.mkOption {
        type = lib.types.str;
        default = "lalt spc tab ralt sys del"; # t480 source thumbRow
      };
    };
    ...
    config = {
      ...
      systemd.user.services = {
        kmonad-miryoku = {
          Unit = {
            Description = "Enable Miryoku lite for KMonad";
          };
          Install = {
          ...
          Service = {
            Type = "exec";
            ExecStart = "${pkgs.kmonad}/bin/kmonad /home/paulm/"+filepath;
            # ${pkgs.kmonad}/bin/kmonad /home/paulm/nixconf/host/laptop/miryoku_kmonad.kbd
            # -i 'device-file "/dev/input/by-path/platform-i8042-serio-0-event-kbd"'
          };
        };
      };

      home.file.miryoku_lite = {
        target = filepath;
        text = ''
          ...
          (defsrc
            ...
            a s d f g h j k l ;
            z x c v b n m , . /
              ${config.feature.kmonad.miryoku.thumbRow}
            '
          )
          (deflayer U_BASE
          q	w	e	r	t	z	u	i	o	p
          ...
          (tap-hold-next-release 200 y (layer-toggle U_BUTTON))	(tap-hold-next-release 200 x ralt)	c	v	b	n	m	,	(tap-hold-next-release 200 . ralt)	(tap-hold-next-release 200 / (layer-toggle U_BUTTON))
